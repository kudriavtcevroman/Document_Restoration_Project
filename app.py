...
restormer.load_state_dict(torch.load('finetuned_restormer.pth', map_location=device))
...
opt = Opt()
gpu_ids = [0] if device.type == 'cuda' else []  # [] на CPU для избежания cuda call
from models.networks import define_G
...
esrgan.load_state_dict(torch.load('finetuned_real_esrgan.pth', map_location=device), strict=False)
